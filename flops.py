...
class SR3():
    ...
    def num_params(self):
        model_parameters = filter(lambda p: p.requires_grad, self.sr3.parameters())
    ...
    def flops(self, input_shape):
        ...
        def conv_hook(module, input, output):
            nonlocal flops
            # For convolutional layers, FLOPs = C_in * C_out * K * K * H_out * W_out
            flops += output.shape[1] * module.out_channels * module.kernel_size[0] * module.kernel_size[1] * output.shape[2] * output.shape[3]

        # Registering hooks to convolutional layers
        for layer in self.sr3.modules():
            if isinstance(layer, nn.Conv2d):
                hook_handle_list.append(layer.register_forward_hook(conv_hook))

        # Perform a forward pass to compute FLOPs
        self.sr3(input)

        # Remove hooks
        for hook in hook_handle_list:
            hook.remove()
...
dataloader = None
testloader = None
schedule_opt = {'schedule':'linear', 'n_timestep':3000, 'linear_start':1e-4, 'linear_end':0.05}
save_path = None
sr3 = SR3(device, img_size, LR_size, loss_type, dataloader, testloader, schedule_opt, save_path)
num_params = sr3.num_params()
input_shape = (1, 3, 256, 256)  # Define the input shape
flops = sr3.flops(input_shape)
